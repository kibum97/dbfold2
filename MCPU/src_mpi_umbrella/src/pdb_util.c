    ...
    for (i = 0; i < natoms; i++)
        if (strcmp(native[i].atomname, "CB") || strcmp(native[i].res, "GLY"))
            fprintf(DATA, "ATOM%7d  %-3s %3s  %4d%12.3f%8.3f%8.3f  1.00  0.00\n", i + 1,
                    native[i].atomname, native[i].res, native[i].res_num + 1, native[i].xyz.x,
                    native[i].xyz.y, native[i].xyz.z);
    fprintf(DATA, "TER\n");
    fprintf(DATA, "END\n");
...
void PrintPDB_Emin(char *filename) {
    ...
    for (i = 0; i < natoms; i++)
        if (strcmp(native[i].atomname, "CB") || strcmp(native[i].res, "GLY"))
            fprintf(DATA, "ATOM%7d  %-3s %3s  %4d%12.3f%8.3f%8.3f  1.00  0.00\n", i + 1,
                    native[i].atomname, native[i].res, native[i].res_num + 1, native_Emin[i].xyz.x,
                    native_Emin[i].xyz.y, native_Emin[i].xyz.z);
    fprintf(DATA, "TER\n");
    fprintf(DATA, "END\n");
...
void PrintPDB_RMSDmin(char *filename) {
    ...
    for (i = 0; i < natoms; i++)
        if (strcmp(native[i].atomname, "CB") || strcmp(native[i].res, "GLY"))
            fprintf(DATA, "ATOM%7d  %-3s %3s  %4d%12.3f%8.3f%8.3f  1.00  0.00\n", i + 1,
                    native[i].atomname, native[i].res, native[i].res_num + 1,
                    native_RMSDmin[i].xyz.x, native_RMSDmin[i].xyz.y, native_RMSDmin[i].xyz.z);
    fprintf(DATA, "TER\n");
    fprintf(DATA, "END\n");
    ...
    if (!strncmp(line, "ATOM", 4)) {
        ...
        if (!USE_GO_POTENTIAL)
            protein[*Natoms].smogtype =
                TypeMaxAtoms(protein[*Natoms].atomname, protein[*Natoms].res);
        else
        ...
        if ((protein[*Natoms].atomtype != -500) && (protein[*Natoms].smogtype != -500) &&
            (USE_SIDECHAINS || !protein[*Natoms].is_sidechain) && (primary_rotamer)) {
            (*Natoms)++;
            if (*Natoms == 1)
