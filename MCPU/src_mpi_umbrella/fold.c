...
void Fold(void) {
    ...
    { /* Initialize contactstring */
        ...
        fill_calpha_contact_string(
            native_residue, struct_native,
            orig_contactstring);  // AB made this change so that this reads structure_file instead
                                  // of native_file...still ok to pass native_residue structure so
                                  // long as structure file has same # of atoms as native file
        // fill_calpha_contact_string (native_residue, orig_native, orig_contactstring);  //can
        // potentially pass struct_native instead of orig_native, while still passing
        // native_residue, since the function just uses native_residue to figure out where each atom
        // is located (at what index)...this should not change so long as I don't change the protein
        // size
    }
    ...
    // align_drms(native, native_residue, struct_native, struct_residue, map_to_seq, map_to_struct,
    // nalign, &native_rms);
    ...
    for (i = 0; i < nalign; ++i) {
        struct_f1[i + 1].CA.x =
            struct_native[struct_residue[map_to_struct[i]].CA]
    ...
    native_rms = getrms(struct_f1, struct_f2, constraint_align);

    ResetEnergies(
        0);  // Here we actually fill in the values for prev_E (which are, ironically, the current
    ...
    fprintf(STATUS,
            ...
            "%.2f(tor) + %.2f(sct)\n\n",
            weight_clash, weight_rms, weight_potential, ARO_WEIGHT, weight_hbond, TOR_WEIGHT,
            SCT_WEIGHT);
    ...
    for (mcstep = 0; mcstep < MC_STEPS; mcstep++) {
        ...
        if (mcstep % MC_REPLICA_STEPS == 0) {
            ...
            if (myrank == 0) {  // node 0 will mediate the exchange
                for (irep = 0; irep < MAX_EXCHANGE; irep++) {
                    ...
                    if (sel_num % NODES_PER_TEMP ==
                        NODES_PER_TEMP - 1) {  // the last setpoint for a given temperature
                    ...
                    delta_N =
                        k_bias *
                            ((Nnode[partner] - Cnode[sel_num]) * (Nnode[partner] - Cnode[sel_num]) -
                             (Nnode[sel_num] - Cnode[sel_num]) *
                                 (Nnode[sel_num] - Cnode[sel_num])) +
                        k_bias *
                            ((Nnode[sel_num] - Cnode[partner]) * (Nnode[sel_num] - Cnode[partner]) -
                             (Nnode[partner] - Cnode[partner]) * (Nnode[partner] - Cnode[partner]));
            ...
            if (mcstep % MC_PRINT_STEPS == 0) {
                fprintf(STATUS,
                        ...
                        mcstep, E, natives, replica_index[myrank], Tnode[replica_index[myrank]],
                        Cnode[replica_index[myrank]], Enode[myrank], Nnode[myrank],
                        accepted_replica[myrank], rejected_replica[myrank]);
            ...
            for (i = 0; i < nprocs;
                ...
                if (replica_index[i] !=
                    ...
                    if (myrank == i) {  // For instance, if my rank is 6 and replica_index[6]=5,
                                        // then I received an exchange from node 5
                        ierr = MPI_Recv(
                            buf_in, 3 * natoms, MPI_FLOAT, replica_index[i], (i + 2),
                            mpi_world_comm,
                            &mpi_status);  // now, receive the atomic coordinates from my partner
                        /* Also receive the "replica number" of the partner */
                        ierr = MPI_Recv(&receive_replicano, 1, MPI_INT, replica_index[i], (i + 3),
                                        mpi_world_comm, &mpi_status);
                    } else if (myrank == replica_index[i]) {  // For instance, if my rank is 5 and
                                                              // replica_index[6]=5, then this means
                                                              // I gave an exchange to node 6
                        ierr = MPI_Send(buf_out, 3 * natoms, MPI_FLOAT, i, (i + 2),
                                        mpi_world_comm);  // give my coordinates to my partner
                        /* Send the "replica number" of this replica */
                        ierr = MPI_Send(&current_replica, 1, MPI_INT, i, (i + 3), mpi_world_comm);
                    }
                }
            }
            ...
            if (replica_index[myrank] !=
                myrank) {  // if exchange occurred, transfer temporary data to real-deal atomic data
        ...
        if (mcstep % MC_PRINT_STEPS == 0) {
            ...
            if (strcmp(constraint_file, "None") != 0) {                                // AB
                ...
                E_constraint_now = Compute_frag_rmsd_constraint_energy(struct_f1, struct_f2,
                                                                       constraint_align);  // AB
            } else {                                                                       // AB
            ...
            fprintf(STATUS,
                    ...
                    "%6.2f %6.3f, %d %6.3f  \n",
                    mcstep, E, ncontacts, native_rms, natives, E_pot_now, E_sct_now, E_hbond_now,
                    E_aro_now, E_tor_now, 100 * (Float)naccepted / (Float)MC_PRINT_STEPS,
                    100 * (Float)nrejected / (Float)MC_PRINT_STEPS,
                    100 * (Float)nothers / (Float)MC_PRINT_STEPS, MC_TEMP,
                    number_of_contacts_setpoint, E_constraint_now);
            fflush(STATUS);
    ...
    fprintf(STATUS,
            ...
            rms_RMSDmin, E_RMSDmin, E_RMSDmin_pot, E_RMSDmin_hbond, E_RMSDmin_tor, E_RMSDmin_sct,
            E_RMSDmin_aro, E_RMSDmin_constraint);
    fprintf(STATUS, "Pdb file at RMSDmin: %s\n", rmsd_filename);

    // The following is added by AB
    // if (myrank<nprocs-1) {
    // fprintf(STATUS, "Final exchange statistics with node %i (T = %5.3f, setpoint=%d):  accepted =
    // %i, rejected = %i \n", myrank+1, Tnode[myrank+1], Cnode[myrank+1],
    // accepted_replica[myrank][myrank + 1], rejected_replica[myrank][myrank + 1]); if (myrank <
    // nprocs- NODES_PER_TEMP) { 	fprintf(STATUS, "Final exchange statistics with node %i (T = %5.3f,
    //setpoint=%d):  accepted = %i, rejected = %i \n", myrank+NODES_PER_TEMP,
    //Tnode[myrank+NODES_PER_TEMP], Cnode[myrank+NODES_PER_TEMP], accepted_replica[myrank][myrank +
    //NODES_PER_TEMP], rejected_replica[myrank][myrank + NODES_PER_TEMP]);
