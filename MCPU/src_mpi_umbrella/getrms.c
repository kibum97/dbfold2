    ...
    for (i = 1; i <= NFRAG; i++) {
        for (j = algn.seqptr[i].x1; j <= algn.seqptr[i].x2; j++) {
            ...
            if (struct1[j].CA.x > -9990 && struct2[k].CA.x > -9990) {
                xm = struct1[j].CA.x - struct2[k].CA.x;
                xp = struct1[j].CA.x + struct2[k].CA.x;
                ym = struct1[j].CA.y - struct2[k].CA.y;
                yp = struct1[j].CA.y + struct2[k].CA.y;
                zm = struct1[j].CA.z - struct2[k].CA.z;
                zp = struct1[j].CA.z + struct2[k].CA.z;
