...
void   loop_Jacobian(double r_n[3][3], double r_ca[3][3], double r_c[3][3], double *Jac) {
    ...
    for (i = 0; i < 3; i++)
        for (k = 0; k < 3;
             k++)  // I think these are just positions of the nitrogens, alpha carbons, and (beta)
                   // carbons for the three AAs ivolved in rotation
        {
            rn[i][k]  = r_n[i][k];
            rca[i][k] = r_ca[i][k];
            rc[i][k]  = r_c[i][k];
...
here:
    ...
    */
    for (i = 0; i < 3; i++) {
        ...
        //     ! psi
        //     n = n + 1
        n++;  // note this, we increase n to not override the axes we just defined, since as
              // mentioned above, each residue will have two "axes"
        //     axis(:,n) = rc(:,i) - rca(:,i)
    ...
    */
    for (n = 0; n < 4;
         n++)  // Note, this Jacobian appears to be transposed relatiive to how it appears in B.9
               // (rows and columns reversed) but this won't affect determininant
    {
        arg_a[0] = r_ca3[0] - pivot[n][0];
        arg_a[1] = r_ca3[1] - pivot[n][1];
        arg_a[2] = r_ca3[2] - pivot[n][2];
        cross(axis[n], arg_a,
              arg_b);  // As per B.7, cross the nth bond vector with the vector from n to final c
                       // alpha...The cross function updates that third element arg_b
        j[n][0] = arg_b[0];
        j[n][1] = arg_b[1];
        j[n][2] = arg_b[2];
    }
    ...
    */
    ...
    //  later those variables c1 and c2 will come into play which dictate the two components that we
    //  actually keep
    ...
    // Negative because it's the sixth element, all even elements in cofactor expansion get negative
    // sign Thus it is only necessary to compute determinaint of that B matrix in Dobbs
    ...
    // I have checked that this expression for the determinant is correct in my (physical paper)
    // notebook, April 14 2019 I used the indexing notation in the code below, which is different
    // from the notation in the comment above (rows/columns reversed)
    ...
    // det3 is a funciton that computes determinant of 3x3 matrix given that that you specify the
    // rows of the matrix as the three arguments
    ...
    if (fabs(det) < 1.0e-10)  // note entirely sure what this if clause is doing, but I imagine it
                              // deals with some numerical underflow?
    {
...
double det3(double j1[3], double j2[3], double j3[3]) {
    ...
    deter = j11 * (j22 * j33 - j23 * j32) - j12 * (j21 * j33 - j23 * j31) +
            j13 * (j21 * j32 - j22 * j31);
