...
void Read_movable_region() {
    ...
    int   n_movable = 0;
    FILE *fp        = fopen(movable_region_file, "r");
    if (fp == NULL) {
        fprintf(STATUS, "Could not open file %s \n", movable_region_file);
        exit(1);
    } else {
        fprintf(STATUS, "Successfully opened movable region file %s \n", movable_region_file);
        fflush(STATUS);
    }
    while (fgets(str, 1000, fp) != NULL) {
        if (str[0] != '#') {
            ...
            for (int i = movable_lower_bound; i < movable_upper_bound; i++) {
                movable_residue_map[n_movable] = i;
                n_movable++;
            }
        }
    }
    ...
    for (int i = 0; i < n_movable; i++) {
        fprintf(STATUS, "DEBUG:Movable residue %d is %d \n", i, movable_residue_map[i]);
        fflush(STATUS);
    }
    n_movable_residues = n_movable;
    return;
}
...
void integloop(float step_size, int *n_soln)  // Does a local move...based on the Dill paper
{
    ...
    *n_soln = 0;  // Number of solutions to 16th degree polynomial
    ...
    sel_res =
        (int)(threefryrand() * n_movable_residues);  // Select a residue to initialize local move
    sel_res = movable_residue_map[sel_res];          // KP editted for movable region constraint
                                                     //  sel_res = 20;
    mc.is_phi = (int)(threefryrand() *
                      2);  // With 50% probability, rotate about phi angle, otherwise psi angle
    //  mc.is_phi = 1;
    // fprintf(STATUS, "%f\n", deg2rad*2.);
    step_size =
        YANG_SCALE * 2. * deg2rad * GaussianNum();  // Magnitude  of driver rotation, in radians
    ...
    // residues are to be formed along (-) direction.
    {
        ...
        if ((native_residue[sel_res].amino_num == 14) ||
            (native_residue[sel_res - 1].amino_num == 14) ||
            (native_residue[sel_res - 2].amino_num == 14) ||
    ...
    // residues are to be formed along (+) direction.
    {
        ...
        if ((native_residue[sel_res + 1].amino_num == 14) ||
            (native_residue[sel_res + 2].amino_num == 14) ||
            (native_residue[sel_res + 3].amino_num == 14)) {
            //      fprintf(STATUS, "Can't rotate phi angle of proline! n0: %d\n", n0);
            return;
    ...
    //    native_residue[s2].res, ang_nac*rad2deg);

    //  if (step%10000 == 0)
    //    fprintf(STATUS, "step:%8d  success:%8d  n0:%4d is_phi: %d  driver ang.:%9.5f  n_soln: %d
    //    ang_nac:%9.5f  ang_side:%9.5f  len_on:%9.5f  len_ns:%9.5f\n", step, success, n0,
    //    mc.is_phi, step_size*rad2deg, *n_soln, ang_nac*rad2deg, ang_side*rad2deg, len_on, len_ns);
...
void yangloop(double r_n[5][3], double r_a[5][3], double r_c[5][3], double r_o[5][3],
    ...
    */
    ...
    soln_no_before = n_soln_before;  // Although n_soln_before is initialized to zero, its value
...
void check_bb() {
    ...
    for (i = 0; i < nresidues - 2; i++) {
        ...
        MakeVector(native[native_residue[i + 2].CA].xyz, native[native_residue[i + 2].N].xyz,
                   &tmp3);
        MakeVector(native[native_residue[i + 2].CA].xyz, native[native_residue[i + 2].O].xyz,
                   &tmp4);
        ...
        MakeVector(native[native_residue[i + 1].N].xyz, native[native_residue[i + 1].CA].xyz,
                   &tmp2);
        MakeVector(native[native_residue[i + 1].CA].xyz, native[native_residue[i + 1].C].xyz,
                   &tmp3);
...
void initialize_aromatic() {
    ...
    for (r = 0; r < nresidues; r++)
        if ((strcmp(native_residue[r].res, "PHE") == 0) ||
            (strcmp(native_residue[r].res, "TRP") == 0) ||
