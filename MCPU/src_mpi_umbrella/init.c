...
char rmsd_constraint_file[500] = "None";
char movable_region_file[500] = "None";
                ...
                if (mu == 1) {
                    ...
                    if (nnat_cons[i][j] != 0)
                        ...
                        potential[i][j] = -1;
                          ...
                          2007*/
                    potential[i][j] = ((1 - mu) * nnat_cons[i][j] - mu * nat_cons[i][j]) /
                                      (mu * nat_cons[i][j] + (1 - mu) * nnat_cons[i][j]);
...
void InitializeProtein() {
    ...
    ReadHelicityData();  // KP: Not being used?
    fprintf(STATUS, "Read Helicity Data\n");

    /* Read data from native_file into native */
    ReadNative(native_file, native,
               &natoms);  // AB: This reads native_file into the structure native
    fprintf(STATUS, "Read Native\n");

    /* Count nresidues */
    nresidues = 0;
    for (i = 0; i < natoms; i++)
        if (!strncmp(native[i].atomname, "CA", 2))
            nresidues++;
    ...
    for (i = 0; i < natoms; i++)
        FindLatticeCoordinates(
            &native[i]); /* this also computes the integer value of the coordinates */

    /* Get residue info */

    native_residue = (struct residue *)calloc(
        nresidues,
    ...
    GetResidueInfo(native, native_residue, nresidues,
                   natoms);  // This adds the necessary info into native_residue
            ...
            if (data[i][j].clashes) {
                if (ReportBack)
                    fprintf(STATUS, "native clash\t%d %s - %s\t%d %s - %s \t %.3f %.3f\n", i,
                            native[i].res, native[i].atomname, j, native[j].res, native[j].atomname,
                            sqrt(D2(native[i].xyz, native[j].xyz)),
                            sqrt(hard_core[native[i].smogtype][native[j].smogtype]) / 100);
...
void GetChi() { /* Chi angles are stored in radians */
    ...
    for (i = 0; i < nresidues; i++)
        for (j = 0; j < amino_acids[native_residue[i].amino_num].ntorsions; j++) {
            native_residue[i].chi[j] =
                PI / 180.0 *
                CalculateTorsion(native, sidechain_torsion[i][j][0], sidechain_torsion[i][j][1],
                                 sidechain_torsion[i][j][2], sidechain_torsion[i][j][3], 0);
...
void ReadAvgChis(void) {
    ...
    //     deviation_ang[native_residue[i].amino_num][2][0],
    //     prob_ang[native_residue[i].amino_num][2]);
    // exit(0);
}
        ...
        if (i > nresidues / 2.0) {
            ...
            for (j = 0; j < natoms; j++)
                if (native[j].res_num < i) {
                    ...
                    if (j != native_residue[i].O && j != native_residue[i].C &&
                        j != native_residue[i].CA)
                        rotate_atom[0][i][rotate_natoms[0][i]++] = j;
                }
        }
    }
            ...
            if (residue_triplets[i].c >= 0) {
                for (k = 0; k < natoms; k++)
                    if (native[k].res_num < residue_triplets[i].c) {
                        ...
                        if (k != native_residue[residue_triplets[i].c].O &&
                            k != native_residue[residue_triplets[i].c].C &&
                            k != native_residue[residue_triplets[i].c].CA)
                            loop_rotate_atoms[i][j][loop_rotate_natoms[i][j]++] = k; /* psi */
                    }
                j += 2;
            } else {
            ...
            if (residue_triplets[i].b >= 0) {
                for (k = 0; k < natoms; k++)
                    if (native[k].res_num < residue_triplets[i].b) {
                        ...
                        if (k != native_residue[residue_triplets[i].b].O &&
                            k != native_residue[residue_triplets[i].b].C &&
                            k != native_residue[residue_triplets[i].b].CA)
                            loop_rotate_atoms[i][j][loop_rotate_natoms[i][j]++] = k; /* psi */
                    }
                j += 2;
            } else {
            ...
            if (loop_rotate_natoms[i][j] && loop_rotate_natoms[i][j - 1]) {
                for (k = 0; k < loop_rotate_natoms[i][j - 1]; k++)
                    if (loop_not_rotated[i][j][loop_rotate_atoms[i][j - 1][k]])
                        loop_int_rotate_atoms[i][j - 1][loop_int_rotate_natoms[i][j - 1]++] =
                            loop_rotate_atoms[i][j - 1][k];
            }
        }
...
void InitializeSidechainRotationData() {
    ...
    for (i = 0; i < nresidues; i++) {
        sidechain_torsion[i] =
            (short **)calloc(amino_acids[native_residue[i].amino_num].ntorsions, sizeof(short *));
        for (j = 0; j < amino_acids[native_residue[i].amino_num].ntorsions; j++)
        ...
        for (j = 0; j < 4; j++)
            for (k = 0; k < 4; k++)
                for (l = 0; l < 4; l++)
                    for (m = 0; m < 4; m++)
                        for (n = 0; n < 4; n++)
                            native_residue[i].avg_angle[j][k][l][m][n] =
                                amino_acids[native_residue[i].amino_num].avg_angle[j][k][l][m][n];

        /* rot_position gives the 4-digit base 3 representation of the rotamer */

        native_residue[i].rot_position =
            (short **)calloc(native_residue[i].nrotamers, sizeof(short *));
    ...
    for (i = 0; i < nresidues; i++) {
        ...
        for (j = 0; j < native_residue[i].ntorsions; j++) {
            rotate_sidechain_atom[i][j] = (short *)calloc(
                amino_acids[native_residue[i].amino_num].rotate_natoms[j], sizeof(short *));
    ...
    for (i = 0; i < nresidues; i++) {
        for (j = 0; j < native_residue[i].ntorsions; j++)
            rotate_sidechain_natoms[i][j] =
                amino_acids[native_residue[i].amino_num].rotate_natoms[j];
        for (j = 0; j < native_residue[i].ntorsions; j++)
            for (k = 0; k < 4; k++) {
                for (l = 0; l < natoms; l++)
                    if (native[l].res_num == i &&
                        !strcmp(native[l].atomname,
                                amino_acids[native_residue[i].amino_num].torsion[j][k])) {
                ...
                if (l == natoms) {
                    fprintf(STATUS, "WARNING -- atom %s, residue %d  %s not found!\n",
                            amino_acids[native_residue[i].amino_num].torsion[j][k], i,
                            native_residue[i].res);
                    exit(1);
                }
            }
        for (j = 0; j < native_residue[i].ntorsions; j++)
            for (k = 0; k < rotate_sidechain_natoms[i][j]; k++) {
                for (l = 0; l < natoms; l++)
                    if (native[l].res_num == i &&
                        !strcmp(native[l].atomname,
                                amino_acids[native_residue[i].amino_num].rotate_atom[j][k])) {
                        rotate_sidechain_atom[i][j][k] = l;
                        break;
                    }
                if (l == natoms) {
                    fprintf(STATUS, "WARNING -- atom %s, residue %d  %s not found!\n",
                            amino_acids[native_residue[i].amino_num].rotate_atom[j][k], i,
                            native_residue[i].res);
                    exit(1);
                }
            }
...
void CheckCorrelation(struct contact_data **Data, struct atom *Protein, struct residue *Residue,
    ...
    for (i = 0; i < Natoms; i++)
        for (j = i + 1; j < Natoms; j++) {
            ...
            else if (fabs(Protein[i].res_num - Protein[j].res_num) <= SKIP_LOCAL_CONTACT_RANGE &&
            ...
            if (!IsSidechainAtom(Protein[i].atomname) && !IsSidechainAtom(Protein[j].atomname) &&
                fabs(Protein[i].res_num - Protein[j].res_num) <= SKIP_BB_CONTACT_RANGE) {
                Data[i][j].check_contacts = 0;
                Data[j][i].check_contacts = 0;
            }
...
void SetProgramOptions(int argc, char *argv[]) {
    ...
    while (fgets(line, 500, DATA) != NULL) {
        ...
        if (strncmp(line, "!", 1)) {
            ...
            if (!strcmp(token, "NATIVE_FILE")) {
                ...
                sscanf(line, "%*s %s", name);
                strcpy(movable_region_file, name);
                ...
                sscanf(line, "%*s %s", name);
                strcpy(rmsd_constraint_file, name);
                ...
                USE_CLUSTER = value;  // note that by default, this value is set to 0 in backbone.h,
                                      // unless you enter something in cfg file
            else if (!strcmp(token, "MAX_CLUSTERSTEP"))  // As of what MC should knowledge based
                                                         // moves be stopped?
                MAX_CLUSTERSTEP = value;  // note that by default, this value is set to 0 in
                ...
                CLUSTER_MOVE = value;          // note that by default, this value is set to 0.33 in
                                               // backbone.h, unless you enter something in cfg file
            else {
    ...
    If we have entered in a native directory, then the files in this directory will override the
    native file that is entered here The files in the native directory need to be in the form
    my_rank.pdb/
    */
    ...
    Suppose nodes_per_temp=5, contacts_max=300, contacts_step=10, mc_temp_min=0.1, temp_step=0.1,
    and n_procs=20 Then the temperautre assignments will be as follows
    ...
    But every nodes_per_temp nodes, we increment the temperature by temp_step, and reset the
    contacts_setpoint to contacts_max
    */
    ...
    */
    ...
    MC_TEMP = Tnode[myrank];  // temperature must always be externally modified if we want ot change
                              // the my_rank_offset
    ...
    if (umbrella == 1) {
        ...
        k_bias = 0;  // regardless of what we had inputted for k_bias, if umbrella is off, we don't
                     // want any bias!!
    ...
    fprintf(STATUS, "myrank : %4d, cfg_file : %s\n k_bias : %5.3f\n setpoint : %d\n ", myrank,
            cfg_file, k_bias, number_of_contacts_setpoint);
    fflush(STATUS);
