...
void SetAlignContactDistance() {
    ...
    if (!USE_GO_POTENTIAL) {
        ...
        for (i = 0; i < MAX_TYPES; i++)
            align_con_dist[i] =
                (struct align_cutoff *)calloc(MAX_TYPES, sizeof(struct align_cutoff));
        ...
        align_con_dist =
            (struct align_cutoff **)calloc(struct_natoms, sizeof(struct align_cutoff *));
        for (i = 0; i < struct_natoms; i++)
            align_con_dist[i] =
                (struct align_cutoff *)calloc(struct_natoms, sizeof(struct align_cutoff));
...
void AlignCheckForContacts(short a, short b) {
    ...
    if (struct_data[a][b].check_clashes &&
        distance < align_hard_core[struct_native[a].smogtype][struct_native[b].smogtype]) {
        struct_data[a][b].clashes = struct_data[b][a].clashes = 1;
        struct_nclashes++;
    }

    if (struct_data[a][b].check_contacts)
        if ((distance <= align_con_dist[struct_native[a].smogtype][struct_native[b].smogtype].b) &&
            (distance >= align_con_dist[struct_native[a].smogtype][struct_native[b].smogtype].a)) {
...
void SetupAlignmentPotential() {
    ...
    for (i = 0; i < nalign; ++i)
        for (j = i + 1; j < nalign; ++j) {
            ...
            same_segment = 0;
            for (m = 0; m < nseg; ++m)
                if ((map_to_struct[i] >= str_segment[m].a) &&
                    (map_to_struct[i] <= str_segment[m].b) &&
                    (map_to_struct[j] >= str_segment[m].a) &&
                    (map_to_struct[j] <= str_segment[m].b))
                    same_segment = 1;
        ...
        for (j = i + 1; j < struct_natoms; ++j) {
            ...
            if ((seqA != -1) && (seqB != -1))
                if (struct_native[i].is_sidechain && struct_native[j].is_sidechain) {
                    same_segment = 0;
                    for (m = 0; m < nseg; ++m)
                        if ((strA >= str_segment[m].a) && (strA <= str_segment[m].b) &&
                            (strB >= str_segment[m].a) && (strB <= str_segment[m].b))
                            same_segment = 1;
