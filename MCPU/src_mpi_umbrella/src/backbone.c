...
int main(int argc, char *argv[]) {
    ...
    InitializeMatrix(); /*AB: Initializes a matrix whose function I'm not totally sure of at the
                           moment*/
    InitializeProtein();
    if (strcmp(rmsd_constraint_file, "None") != 0) {
        Init_frag_rmsd_constraints();
    }
    if (strcmp(movable_region_file, "None") != 0) {  // KP added for movable region constraint
        Read_movable_region();
    } else {
        n_movable_residues = nresidues;
        for (i = 0; i < nresidues; i++) {
            movable_residue_map[i] = i;
        }
    }
    for (i = 0; i < nresidues; i++)
    ...
    fprintf(STATUS,
            "Note that if any secondary structure character is anything except C, then the value "
            "of USE_CLUSTER specified in the cfg file will be overwritten by a default value (see "
            "move.h). \n This may lead to the implementation of (detailed-balance violating) "
            "knowledge moves!  \n");
