...
char aa_singlet[] = "ARNDCQEGHILKMFPSTWYVX";  // X for linker/unknown
...
int main(int argc, char *argv[]) {
    ...
    for (i = 0; i < n - 2; i++) {
        ...
        while (fgets(line, 200, f_org) != NULL) {
            sscanf(line, "%d%d%d %d%d%d%d %d %d %d %*s", &res1, &res2, &res3, &ang_PCA, &ang_bCA,
                   &phim, &psim, &energy, &obs, &tot_obs);
            // printf("%s", line);
            // printf("res1 = %d, res2 = %d, res3 = %d\n",res1, res2, res3);
            if ((r1 == res1) && (r2 == res2) && (r3 == res3))
                fprintf(f_tar, "%4d %2d %2d %2d %3d %3d %3d %3d %5d %7d %7d\n", i, res1, res2, res3,
                        ang_PCA, ang_bCA, phim, psim, energy, obs, tot_obs);
        }
    ...
    for (i = 0; i < n - 3; i++) {
        ...
        while (fgets(line, 200, f_org) != NULL) {
            sscanf(line, "%d%d%d %d%d%d%d %d %d %d %*s", &res1, &res2, &res3, &ang[0], &ang[1],
                   &ang[2], &ang[3], &energy, &obs, &tot_obs);
            if ((r1 == res1) && (r2 == res2) && (r3 == res3))
                fprintf(f_tar, "%4d %2d %2d %2d %3d %3d %3d %3d %5d %7d %7d\n", i, res1, res2, res3,
                        ang[0], ang[1], ang[2], ang[3], energy, obs, tot_obs);
        }
        fclose(f_org);
    }
