...
void dRms(struct atom *protein1, struct atom *protein2, int num_atoms, Float *sc_rms,
          Float *bb_rms) {
...
void align_drms(struct atom *protein1, struct residue *residue1, struct atom *protein2,
                struct residue *residue2, int *map1, int *map2, int align_len, Float *rms) {
...
void SSdRms(struct atom *protein1, struct atom *protein2, int num_atoms, Float *sc_rms,
            Float *bb_rms, int first_res, int last_res) {
    Float scrms_tot, bbrms_tot, d1, d2;
    int   sc_count = 0, ca_count = 0, i, j;
    ...
    for (i = 0; i < num_atoms; ++i)
        for (j = i + 1; j < num_atoms; ++j) {
            if ((protein1[i].res_num >= first_res) && (protein1[i].res_num <= last_res) &&
                (protein1[j].res_num >= first_res) && (protein1[j].res_num <= last_res)) {
                if ((protein1[i].is_sidechain) && (protein1[j].is_sidechain)) {
                    d1 = sqrt(D2(protein1[i].xyz, protein1[j].xyz));
                    d2 = sqrt(D2(protein2[i].xyz, protein2[j].xyz));
                    scrms_tot += (d1 - d2) * (d1 - d2);
                    ++sc_count;
                } else if (!strcmp(protein1[i].atomname, "CA") &&
                           !strcmp(protein1[j].atomname, "CA")) {
                    d1 = sqrt(D2(protein1[i].xyz, protein1[j].xyz));
                    d2 = sqrt(D2(protein2[i].xyz, protein2[j].xyz));
                    bbrms_tot += (d1 - d2) * (d1 - d2);
                    ++ca_count;
...
Float Phi(struct residue current_residue, struct residue prev_residue, struct atom *protein) {
    return CalculateTorsion(protein, prev_residue.C, current_residue.N, current_residue.CA,
                            current_residue.C, 0);
}

Float Psi(struct residue current_residue, struct residue next_residue, struct atom *protein) {
    return CalculateTorsion(protein, next_residue.N, current_residue.C, current_residue.CA,
                            current_residue.N, 0);
}
