...
void poly_mul_sub2(double u1[5][5], double u2[5][5], double u3[5][5],
                   double u4[5][5], int p1[2], int p2[2], int p3[2], int p4[2],
...
void poly_sub1(double u1[17], double u2[17], int p1, int p2, double u3[17],
               int *p3);
void coord_from_poly_roots(int *n_soln, double roots[max_soln], double r_n1[3],
                           double r_a1[3], double r_a3[3], double r_c3[3],
...
void c_dih_ang(double r1[3], double r2[3], double r3[3], double r4[3],
               double *angle);
float struct_dih_ang(struct vector r1, struct vector r2, struct vector r3,
                     struct vector r4);
...
void get_rot(double obj_i[3], double obj_f[3], double tor1[3], double tor2[3],
             double tor3[3], double tor4_i[3], double tor_f[3]);
void driver_rot(double obj_i[3], double obj_f[3], double tor2[3],
                double tor3[3], float dih_change);
//!----------------------------------------------------------------------
//! Copyright (C) 2003
...
//  real(dp), parameter :: two_pi=2.0d0*pi, deg2rad = pi/180.0d0, rad2deg =
//  180.0d0/pi
...
void matmul(double ma[3][3], double mb[3], double mc[3]) {
  ...
  for (i = 0; i < 3; i++) {
    mc[i] = 0.;
    for (j = 0; j < 3; j++) mc[i] += ma[i][j] * mb[j];
  }
  return;
}
...
void solve_3pep_poly(double r_n1[3], double r_a1[3], double r_a3[3],
  ...
  //  call coord_from_poly_roots(n_soln, roots, r_n1, r_a1, r_a3, r_c3,
  //  r_soln_n, r_soln_a, r_soln_c)
  coord_from_poly_roots(n_soln, roots, r_n1, r_a1, r_a3, r_c3, r_soln_n,
                        r_soln_a, r_soln_c);
...
void initialize_loop_closure(double b_len[6], double b_ang[7],
  ...
  //! i.e. the fixed values! Namely the fixed omega dihedral angle and known
  //! bond lengths and atomic angles based on basic chemistry, orbitals etc
  //!-----------------------------------------------------------------------
  //  implicit none
  //  real(dp), intent(in) :: b_len(6), b_ang(7), t_ang(2)
  //  real(dp) :: len1, len2, a_min, a_max
  double len1, len2, a_min, a_max;
  //  real(dp), dimension(3) :: axis, rr_a1, rr_c1, rr_n2, rr_a2, rr_n2a2_ref,
  //  rr_c1a1
  double axis[3], rr_a1[3], rr_c1[3], rr_n2[3], rr_a2[3], rr_n2a2_ref[3],
      rr_c1a1[3];
  ...
  //  len0(1:6) = b_len(1:6)
  for (i = 0; i < 6; i++) len0[i] = b_len[i];
  //  b_ang0(1:7) = b_ang(1:7)
  for (i = 0; i < 7; i++) b_ang0[i] = b_ang[i];
  //  t_ang0(1:2) = t_ang(1:2)
  for (i = 0; i < 2; i++) t_ang0[i] = t_ang[i];

  //  rr_c1(1:3) = 0.0d0
  for (i = 0; i < 3; i++) rr_c1[i] = 0.;
  ...
  {
    //     rr_a1(1:3) = (/ cos(b_ang0(3*i+2))*len0(3*i+1),
    //     sin(b_ang0(3*i+2))*len0(3*i+1), 0.0d0 /)
    // Get various vectors
    rr_a1[0] =
        cos(b_ang0[3 * i + 1]) *
    ...
    rr_n2[2] = 0.0e0;
    //     rr_c1a1(:) = rr_a1(:) - rr_c1(:)
    for (j = 0; j < 3; j++) rr_c1a1[j] = rr_a1[j] - rr_c1[j];
  ...
  //  a_max*rad2deg); aa13_min_sqr = len_aa(2)**2 + len_aa(3)**2
  //  - 2.0d0*len_aa(2)*len_aa(3)*cos(a_min)
  aa13_min_sqr = pow(len_aa[1], 2) + pow(len_aa[2], 2) -
                 2.0e0 * len_aa[1] * len_aa[2] * cos(a_min);
  //  aa13_max_sqr = len_aa(2)**2 + len_aa(3)**2
  //  - 2.0d0*len_aa(2)*len_aa(3)*cos(a_max)
  aa13_max_sqr = pow(len_aa[1], 2) + pow(len_aa[2], 2) -
                 2.0e0 * len_aa[1] * len_aa[2] * cos(a_max);
...
void get_input_angles(int *n_soln, double r_n1[3], double r_a1[3],
  ...
  *n_soln = max_soln;
  ...
  //  ! vertual bond
  //  r_a1a3(:) = r_a3(:) - r_a1(:)
  for (i = 0; i < 3; i++) r_a1a3[i] = r_a3[i] - r_a1[i];
  ...
  //  ! bond lengths
  //  r_a1n1(:) = r_n1(:) - r_a1(:)
  for (i = 0; i < 3; i++) r_a1n1[i] = r_n1[i] - r_a1[i];
  ...
  len_na[2] = len0[5];
  //  r_a3c3(:) = r_c3(:) - r_a3(:)
  for (i = 0; i < 3; i++) r_a3c3[i] = r_c3[i] - r_a3[i];
  //  len_ac(1) = len0(1)
  len_ac[0] = len0[0];
  //  len_ac(2) = len0(4)
  len_ac[1] = len0[3];
  ...
  for (i = 0; i < 3; i++) tmp_val[i] = -b_a1n1[i];
  ...
  for (i = 0; i < 3; i++) tmp_val[i] = -b_a1a3[i];
  ...
  //  end do
  for (i = 0; i < 3; i++) cos_theta[i] = cos(theta[i]);
...
void test_two_cone_existence_soln(double tt, double kx, double et, double ap,
  ...
  //  real(dp) :: cos_tx1, cos_tx2, cos_te1, cos_te2, cos_ea1, cos_ea2, cos_xa1,
  //  cos_xa2
  ...
  *n_soln = max_soln;
  ...
  if (complicated) {
    ...
    t2 = 0;
    //    if ((cos_te1-cos_xa2)*(cos_te1-cos_xa1) <= 0.0d0) s1 = .true.
    if ((cos_te1 - cos_xa2) * (cos_te1 - cos_xa1) <= 0.0e0) s1 = 0;
    //    if ((cos_te2-cos_xa2)*(cos_te2-cos_xa1) <= 0.0d0) s2 = .true.
    if ((cos_te2 - cos_xa2) * (cos_te2 - cos_xa1) <= 0.0e0) s2 = 0;
    //    if ((cos_tx1-cos_ea2)*(cos_tx1-cos_ea1) <= 0.0d0) t1 = .true.
    if ((cos_tx1 - cos_ea2) * (cos_tx1 - cos_ea1) <= 0.0e0) t1 = 0;
    //    if ((cos_tx2-cos_ea2)*(cos_tx2-cos_ea1) <= 0.0d0) t2 = .true.
    if ((cos_tx2 - cos_ea2) * (cos_tx2 - cos_ea1) <= 0.0e0) t2 = 0;
  }
...
void get_poly_coeff(double poly_coeff[deg_pol + 1]) {
  ...
  double um1[5][5], um2[5][5], um3[5][5], um4[5][5], um5[5][5], um6[5][5],
      q_tmp[5][5];
  //  integer, dimension(2) :: p1, p3, p_um1, p_um2, p_um3, p_um4, p_um5, p_um6,
  //  p_Q
  int p1[2], p3[2], p_um1[2], p_um2[2], p_um3[2], p_um4[2], p_um5[2], p_um6[2],
      p_Q[2];
  ...
  double f10[17], f11[17], f12[17], f13[17], f14[17], f15[17], f16[17], f17[17],
      f18[17];
  ...
  for (i = 0; i < 5; i++)
    for (j = 0; j < 5; j++) Q[i][j] = q_tmp[i][j];

  //  ! second determinant
  //  R(:,:) = 0.0d0
  for (i = 0; i < 3; i++)
    for (j = 0; j < 17; j++) R[i][j] = 0.;
  //  R(0:2,0) = C0(0:2,3)
  //  R(0:2,1) = C1(0:2,3)
  //  R(0:2,2) = C2(0:2,3)
  for (i = 0; i < 3; i++) {
  ...
  //  call poly_mul_sub1(R(:,1), R(:,1), R(:,0), R(:,2), p2, p2, p2, p2, f1,
  //  p_f1)
  ...
  //  call poly_mul_sub1(Q(:,1), R(:,1), Q(:,0), R(:,2), p4, p2, p4, p2, f6,
  //  p_f6)
  ...
  //  call poly_mul_sub1(Q(:,3), R(:,1), Q(:,4), R(:,0), p4, p2, p4, p2, f10,
  //  p_f10)
  poly_mul_sub1(Q[3], R[1], Q[4], R[0], p4, p2, p4, p2, f10, &p_f10);
  //  call poly_mul_sub1(Q(:,2), f1, R(:,0), f10, p4, p_f1, p2, p_f10, f11,
  //  p_f11)
  poly_mul_sub1(Q[2], f1, R[0], f10, p4, p_f1, p2, p_f10, f11, &p_f11);
  //  call poly_mul_sub1(Q(:,1), f3, R(:,0), f11, p4, p_f3, p2, p_f11, f12,
  //  p_f12)
  poly_mul_sub1(Q[1], f3, R[0], f11, p4, p_f3, p2, p_f11, f12, &p_f12);

  //  call poly_mul_sub1(Q(:,2), R(:,1), Q(:,1), R(:,2), p4, p2, p4, p2, f13,
  //  p_f13)
  poly_mul_sub1(Q[2], R[1], Q[1], R[2], p4, p2, p4, p2, f13, &p_f13);
  //  call poly_mul_sub1(Q(:,3), f1, R(:,2), f13, p4, p_f1, p2, p_f13, f14,
  //  p_f14)
  poly_mul_sub1(Q[3], f1, R[2], f13, p4, p_f1, p2, p_f13, f14, &p_f14);
  //  call poly_mul_sub1(Q(:,3), R(:,1), Q(:,2), R(:,2), p4, p2, p4, p2, f15,
  //  p_f15)
  poly_mul_sub1(Q[3], R[1], Q[2], R[2], p4, p2, p4, p2, f15, &p_f15);
  //  call poly_mul_sub1(Q(:,4), f1, R(:,2), f15, p4, p_f1, p2, p_f15, f16,
  //  p_f16)
  poly_mul_sub1(Q[4], f1, R[2], f15, p4, p_f1, p2, p_f15, f16, &p_f16);
  //  call poly_mul_sub1(Q(:,1), f14, Q(:,0), f16, p4, p_f14, p4, p_f16, f17,
  //  p_f17)
  poly_mul_sub1(Q[1], f14, Q[0], f16, p4, p_f14, p4, p_f16, f17, &p_f17);

  //  call poly_mul_sub1(Q(:,2), R(:,2), Q(:,3), R(:,1), p4, p2, p4, p2, f18,
  //  p_f18)
  poly_mul_sub1(Q[2], R[2], Q[3], R[1], p4, p2, p4, p2, f18, &p_f18);
  //  call poly_mul_sub1(Q(:,1), R(:,2), Q(:,3), R(:,0), p4, p2, p4, p2, f19,
  //  p_f19)
  poly_mul_sub1(Q[1], R[2], Q[3], R[0], p4, p2, p4, p2, f19, &p_f19);
  //  call poly_mul_sub1(Q(:,3), f19, Q(:,2), f18, p4, p_f19, p4, p_f18, f20,
  //  p_f20)
  poly_mul_sub1(Q[3], f19, Q[2], f18, p4, p_f19, p4, p_f18, f20, &p_f20);
  //  call poly_mul_sub1(Q(:,1), R(:,1), Q(:,2), R(:,0), p4, p2, p4, p2, f21,
  //  p_f21)
  ...
  //  call poly_mul_sub1(Q(:,4), f12, R(:,2), f25, p4, p_f12, p2, p_f25, f26,
  //  p_f26)
  poly_mul_sub1(Q[4], f12, R[2], f25, p4, p_f12, p2, p_f25, f26, &p_f26);
  //  call poly_mul_sub1(Q(:,0), f9, R(:,0), f26, p4, p_f9, p2, p_f26,
  //  poly_coeff, p_final)
  ...
  if (poly_coeff[16] < 0.0e0)
    for (i = 0; i < 17; i++) poly_coeff[i] *= -1.0;
  ...
  if (print_level > 0) {
    ...
    for (i = 0; i < 17; i++) fprintf(STATUS, "%5d%15.6f\n", i, poly_coeff[i]);
  }
...
void poly_mul2(double u1[5][5], double u2[5][5], int p1[2], int p2[2],
  ...
  //  p3(:) = p1(:) + p2(:)
  for (i = 0; i < 2; i++) p3[i] = p1[i] + p2[i];
  for (i = 0; i < 5; i++)
    for (j = 0; j < 5; j++) u3[i][j] = 0.0e0;
...
//!----------------------------------------------------------------------------
// subroutine coord_from_poly_roots(n_soln, roots, r_n1, r_a1, r_a3, r_c3,
// r_soln_n, r_soln_a, r_soln_c)
void coord_from_poly_roots(int *n_soln, double roots[max_soln], double r_n1[3],
                           double r_a1[3], double r_a3[3], double r_c3[3],
                           double r_soln_n[max_soln][3][3],
  ...
  //  real(dp) :: p_s_c(3,3), s1_s(3,3), s2_s(3,3), p_t_c(3,3), t1_s(3,3),
  //  t2_s(3,3)
  double p_s_c[3][3], s1_s[3][3], s2_s[3][3], p_t_c[3][3], t1_s[3][3],
      t2_s[3][3];
  //  real(dp) :: angle, sig1_init, half_tan(3)
  double angle, sig1_init, half_tan[3];
  //  real(dp) :: cos_tau(0:3), sin_tau(0:3), cos_sig(3), sin_sig(3), ht, tmp,
  //  sig1
  double cos_tau[4], sin_tau[4], cos_sig[3], sin_sig[3], ht, tmp, sig1;
  //  real(dp) :: r_s(3), r_t(3), r0(3), r_n(3,3), r_a(3,3), r_c(3,3), p(4),
  //  Us(3,3)
  ...
  //  real(dp) :: rr_a1c1(3), rr_c1n2(3), rr_n2a2(3), rr_a2c2(3), rr_c2n3(3),
  //  rr_n3a3(3), rr_a1a2(3), rr_a2a3(3)
  double rr_a1c1[3], rr_c1n2[3], rr_n2a2[3], rr_a2c2[3], rr_c2n3[3], rr_n3a3[3],
      rr_a1a2[3], rr_a2a3[3];
  ...
  for (i_soln = 0; i_soln < *n_soln; i_soln++) {
    ...
    //     cos_tau[2], cos_tau[3]); fprintf(STATUS,"sin:
    //     %7.3f%7.3f%7.3f%7.3f\n", sin_tau[0], sin_tau[1], sin_tau[2],
    //     sin_tau[3]); do i = 1, 3
    //        j = i - 1
    ...
    for (i = 0; i < 3; i++)
      for (j = 0; j < 3; j++) {
        r_s[j] =
            p_s_c[i][j] + cos_sig[i] * s1_s[i][j] + sin_sig[i] * s2_s[i][j];
        r_t[j] = p_t_c[i][j] + cos_tau[i + 1] * t1_s[i][j] +
                 sin_tau[i + 1] * t2_s[i][j];
    ...
    if (print_level > 0) {
      ...
      for (i = 0; i < 3; i++) tmp_array[i] = rr_a1c1[i] / a1c1;
      calc_bnd_ang(b_a1n1, tmp_array, &n1a1c1);
      //        call calc_bnd_ang(b_a3c3, -rr_n3a3/n3a3, n3a3c3)
      for (i = 0; i < 3; i++) tmp_array[i] = -rr_n3a3[i] / n3a3;
      ...
      //        write(*,"('ang_nac = ', 2f9.3)") b_ang0(1)*rad2deg,
      //        n1a1c1*rad2deg
      fprintf(STATUS, "ang_nac = %9.3f%9.3f\n", b_ang0[0] * rad2deg,
              n1a1c1 * rad2deg);
      //        write(*,"('ang_nac = ', 2f9.3)") b_ang0(4)*rad2deg,
      //        n2a2c2*rad2deg
      fprintf(STATUS, "ang_nac = %9.3f%9.3f\n", b_ang0[3] * rad2deg,
              n2a2c2 * rad2deg);
      //        write(*,"('ang_nac = ', 2f9.3)") b_ang0(7)*rad2deg,
      //        n3a3c3*rad2deg
      fprintf(STATUS, "ang_nac = %9.3f%9.3f\n", b_ang0[6] * rad2deg,
              n3a3c3 * rad2deg);

      //        call calc_dih_ang(rr_a1c1/a1c1, rr_c1n2/c1n2, rr_n2a2/n2a2,
      //        a1c1n2a2)
      for (i = 0; i < 3; i++) {
      ...
      //        call calc_dih_ang(rr_a2c2/a2c2, rr_c2n3/c2n3, rr_n3a3/n3a3,
      //        a2c2n3a3)
      for (i = 0; i < 3; i++) {
        tmp_array1[i] = rr_a2c2[i] / a2c2;
      ...
      //        write(*,"('t_ang1 = ', 2f9.3)") t_ang0(1)*rad2deg,
      //        a1c1n2a2*rad2deg
      fprintf(STATUS, "t_ang1 = %9.3f%9.3f\n", t_ang0[0] * rad2deg,
              a1c1n2a2 * rad2deg);
      //        write(*,"('t_ang2 = ', 2f9.3)") t_ang0(2)*rad2deg,
      //        a2c2n3a3*rad2deg
...
double calc_t2(double t0) {
  ...
  A0 =
      Q[0][0] + Q[0][1] * t0 + Q[0][2] * t0_2 + Q[0][3] * t0_3 + Q[0][4] * t0_4;
  //  A1 = Q(0,1) + Q(1,1)*t0 + Q(2,1)*t0_2 + Q(3,1)*t0_3 + Q(4,1)*t0_4
  A1 =
      Q[1][0] + Q[1][1] * t0 + Q[1][2] * t0_2 + Q[1][3] * t0_3 + Q[1][4] * t0_4;
  //  A2 = Q(0,2) + Q(1,2)*t0 + Q(2,2)*t0_2 + Q(3,2)*t0_3 + Q(4,2)*t0_4
  A2 =
      Q[2][0] + Q[2][1] * t0 + Q[2][2] * t0_2 + Q[2][3] * t0_3 + Q[2][4] * t0_4;
  //  A3 = Q(0,3) + Q(1,3)*t0 + Q(2,3)*t0_2 + Q(3,3)*t0_3 + Q(4,3)*t0_4
  A3 =
      Q[3][0] + Q[3][1] * t0 + Q[3][2] * t0_2 + Q[3][3] * t0_3 + Q[3][4] * t0_4;
  //  A4 = Q(0,4) + Q(1,4)*t0 + Q(2,4)*t0_2 + Q(3,4)*t0_3 + Q(4,4)*t0_4
  A4 =
      Q[4][0] + Q[4][1] * t0 + Q[4][2] * t0_2 + Q[4][3] * t0_3 + Q[4][4] * t0_4;
...
//!-----------------------------------------------------------------------
float struct_calc_dih_ang(struct vector r1, struct vector r2,
                          struct vector r3) {
  struct vector p, q, s;
  float arg, angle;
...
void rotation_matrix(double q[4], double U[3][3]) {
  ...
  //  real(dp) ::
  //  q0,q1,q2,q3,b0,b1,b2,b3,q00,q01,q02,q03,q11,q12,q13,q22,q23,q33
  double q0, q1, q2, q3, b0, b1, b2, b3, q00, q01, q02, q03, q11, q12, q13, q22,
      q23, q33;
...
void get_rot(double obj_i[3], double obj_f[3], double tor1[3], double tor2[3],
             double tor3[3], double tor4_i[3], double tor4_f[3]) {
  ...
  */
  ...
  for (i = 0; i < 3; i++) vec_rot[i] = (tor3[i] - tor2[i]) / vec_len;

  quaternion(vec_rot, angle, p);
  rotation_matrix(p, Us);

  for (i = 0; i < 3; i++) mat11[i] = obj_i[i] - tor3[i];
  matmul(Us, mat11, mat1);
  for (i = 0; i < 3; i++) obj_f[i] = mat1[i] + tor3[i];
