...
struct alignment constraint_align = {0};
...
void Init_frag_rmsd_constraints() {
    ...
    FILE *fp    = fopen(rmsd_constraint_file, "r");
    int   nfrag = 0;
    if (fp == NULL) {
        fprintf(STATUS, "Could not open file %s \n", rmsd_constraint_file);
        exit(1);
    } else {
...
float Compute_frag_rmsd_constraint_energy(struct backbone  struct1[MAXSEQUENCE],
    ...
    return k_constraint * frag_rmsd * frag_rmsd;
}
...
void Read_constraints() {
    ...
    // Am using Method 2 under dynamically allocated 2D arrays (but changed Malloc to Calloc based
    // on some advantages I read online)

    // We already declare the following in backbone.h
    // int Max_N_constraints = 1000;
    // int N_constraints;
    // int **constraint_array; //We will declare a pointer to an array of Max_N_constraints pointers
    // to ints...the ** indicates pointer to a pointer

    // float *constraint_weights; //A pointer to an array

    constraint_array = (int **)calloc(
        Max_N_constraints,
        sizeof(
    ...
    constraint_weights = (float *)calloc(Max_N_constraints,
    ...
    while (fgets(str, 1000, fp) != NULL) {
        ...
        if (str[0] != '#') {
            ...
            constraint_array[iii] = (int *)calloc(
    ...
    if (N_constraints > 0) {
        fprintf(STATUS, "We have %i constraints. The 1st constraint is %i, %i with weight %f \n",
                N_constraints, constraint_array[0][0], constraint_array[0][1],
                constraint_weights[0]);
        fflush(STATUS);
    }

    distances =
        (float *)calloc(N_constraints, sizeof(float));  // Will contain distance between i and j
    disulfide_pairs_attempt = (int *)calloc(
        N_constraints,
...
float Compute_constraint_energy(const struct residue *residues, const struct atom *atoms) {
    ...
    for (i = 0; i < N_constraints; i++) {
        ...
        distances[i] = dist;
        /* Note we don't use this var */
        disulfide_pairs_attempt[i] = 0;  // for now, reset the value of disulfide..we will figure
        ...
        if (dist <= 2.9) {
            constraint_energy += weight * k_constraint * ((dist - 2.9) * (dist - 2.9) - 1);
        } else if (dist <= 8.) {
            constraint_energy += weight * k_constraint * -1;
        } else { /* (dist > 8.) */
            constraint_energy += weight * k_constraint * ((dist - 8.) * (dist - 8.) - 1);
        }
    }
