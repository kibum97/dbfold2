...
#include "define.h"      // For Float, constants like CLUSTER_MOVE
...
float USE_CLUSTER =
    0.0;  // 0.0 to turn knowledge based moves off, was 0.1 originally (knowledge moves on)
int MAX_CLUSTERSTEP =
    0;  // Added by AB...all MC steps after this will always have USE_CLUSTER set to 0
    ...
    do { /* See the last line of this clause, while (sidechain_step++ < SIDECHAIN_MOVES)
        ...
        if (sidechain_step == 0) {
            ...
            if ((threefryrand() < CLUSTER_MOVE) &&
                USE_CLUSTER) {  // Cluster_move will often be set to 0 to suppress knowledge moves
                // fprintf(STATUS, "LoopBackbondMove():\n");
                LoopBackboneMove(step_size);  // Make a knowledge based backbone move
                use_yang = 0;
            } else {
                if (YANG_MOVE &&
                    threefryrand() < YANG_MOVE) {   // Make a local move, as in Dill paper
            ...
            if (rmsd_constraint > 0) {
                ...
                new_rms = getrms(struct_f1, struct_f2, constraint_align);
            }
         ...
         */
            ...
            if (mc_flags.init)
                for (i = 0; i < total_pairs2; i++)
                    delta_nclashes +=
                        data[cd[i].a][cd[i].b].delta_clashes - data[cd[i].a][cd[i].b].clashes;
            ...
            delta_contacts = 0;
            for (i = 0; i < total_pairs; i++) { /*AB: Loop through all pairs of atoms and compute
                                                   potential change due to move*/
                N   = ab[i].a;
                M   = ab[i].b; /*N, M are indices for pairs of atoms*/
                del = data[N][M].delta_contacts -
                      data[N][M].contacts; /*I think del indicates somethign like how many new
                                              contacts have formed?*/
                delta_contacts += del;
                dE_pot += ((Float)del) *
                          potential[native[N].smogtype]
                                   [native[M].smogtype]; /*Compute change in mu potential
            ...
            if (strcmp(constraint_file, "None") != 0) {  // AB
                dE_constraint =
                    Compute_constraint_energy(native_residue, native) - prev_E_constraint;  // AB
            } else if (strcmp(rmsd_constraint_file, "None") != 0) {
                dE_constraint =
                    Compute_frag_rmsd_constraint_energy(struct_f1, struct_f2, constraint_align) -
                    prev_E_constraint;  // AB
            }  // AB: Otherwise it is 0, as previously initialized

            dE = weight_potential * dE_pot + weight_clash * delta_nclashes +
                 weight_hbond * dE_hbond + TOR_WEIGHT * dE_tor + ARO_WEIGHT * dE_aro +
                 SCT_WEIGHT * dE_sct + dE_constraint;  // AB addded last term
            double arg = dE / MC_TEMP;
            /* wmj ****************************** */

            new_natives =
                number_of_calpha_native_contacts(native_residue, native, orig_contactstring);
            diff_number_of_contacts_new = new_natives - number_of_contacts_setpoint;
            double dbias =
                k_bias * (diff_number_of_contacts_new * diff_number_of_contacts_new -
            ...
            if (use_yang == 1) {
                acceptance_crit =
                    acceptance_crit * (n_soln / (double)soln_no_before) *
                    (jacobi_after / jacobi_before);  // AB changed to obey detailed balance in case
                                                     // local moves are used
...
void LocalBackboneMove(Float step_size) {
    ...
    NewDeltaContacts(rotate_natoms[mc.is_phi][mc.sel_res_num],
                     rotate_atom[mc.is_phi][mc.sel_res_num],
                     &not_rotated[mc.is_phi][mc.sel_res_num]);
    return;
}
...
void LoopBackboneMove(Float absolute_step_size) {
    ...
    /* triplet is selected */
    if (0 == 0) {
        mc.sel_triplet = (int)(threefryrand() * TOTAL_TRIPLE_LOOP_MOVES) + TOTAL_SINGLE_LOOP_MOVES +
                         TOTAL_DOUBLE_LOOP_MOVES;
    ...
    if (mc.selected[i] > nresidues / 2.0) {
        if (mc.selected[i] != 0 && mc.selected[i] != (nresidues - 1) &&
            native_residue[mc.selected[i]].amino_num != 14) {
            ...
            DoRotation(a, b, c, d, step_phi, loop_rotate_natoms[mc.selected[i]][0],
                       loop_rotate_atoms[mc.selected[i]][0]);
        ...
        if (mc.selected[i] != 0 && mc.selected[i] != (nresidues - 1)) {
            ...
            DoRotation(a, b, c, d, step_psi, loop_rotate_natoms[mc.selected[i]][1],
                       loop_rotate_atoms[mc.selected[i]][1]);
        ...
        if (mc.selected[i] != 0 && mc.selected[i] != (nresidues - 1)) {
            ...
            d = native_residue[mc.selected[i] + 1].N;
            DoRotation(a, b, c, d, -step_psi, loop_rotate_natoms[mc.selected[i]][0],
                       loop_rotate_atoms[mc.selected[i]][0]);
            mc.delta_psi_angle[i] = -step_psi;
            for (j = 0; j < loop_rotate_natoms[mc.selected[i]][0]; j++) {
                is_rotated[loop_rotate_atoms[mc.selected[i]][0][j]] = 1;
        ...
        if (mc.selected[i] != 0 && mc.selected[i] != (nresidues - 1) &&
            ...
            b = native_residue[mc.selected[i]].CA;
            d = native_residue[mc.selected[i]].C;
            DoRotation(a, b, c, d, -step_phi, loop_rotate_natoms[mc.selected[i]][1],
                       loop_rotate_atoms[mc.selected[i]][1]);
            mc.delta_phi_angle[i] = -step_phi;
            for (j = 0; j < loop_rotate_natoms[mc.selected[i]][1]; j++) {
                is_rotated[loop_rotate_atoms[mc.selected[i]][1][j]] = 2;
    ...
    NewDeltaContacts(loop_rotate_natoms[mc.selected[i]][0], loop_rotate_atoms[mc.selected[i]][0],
                     &loop_not_rotated[mc.selected[i]][0]);
    return;
}
...
void BackboneMove(Float step_size) {
    ...
    DoRotation(a, b, c, d, mc.delta_angle[0], rotate_natoms[mc.is_phi][mc.sel_res_num],
               rotate_atom[mc.is_phi][mc.sel_res_num]);
...
void MakeSidechainMove() {
    ...
    for (i = 0; i < native_residue[mc.sel_res_num].ntorsions; i++) {
        ...
        //           sel_no_chi, rotamer_angles[mc.sel_res_num].chis[sel_no_chi][i],
        //           deviation_ang[native_residue[mc.sel_res_num].amino_num][sel_no_chi][i],
        //	   p_0to1, cummul_prob,
        //prob_ang[native_residue[mc.sel_res_num].amino_num][sel_no_chi]);

        if (USE_ROTAMERS)
            mc.delta_angle[i] =
                rotamer_angles[mc.sel_res_num].chis[sel_no_chi][i] +
                GaussianNum() *
                    deviation_ang[native_residue[mc.sel_res_num].amino_num][sel_no_chi][i] -
                native_residue[mc.sel_res_num].chi[i];
        else
            mc.delta_angle[i] = GaussianNum() * SIDECHAIN_NOISE;
        native_residue[mc.sel_res_num].tmpchi[i] =
            native_residue[mc.sel_res_num].chi[i] + mc.delta_angle[i];

        DoRotation(a, b, c, d, mc.delta_angle[i], rotate_sidechain_natoms[mc.sel_res_num][i],
                   rotate_sidechain_atom[mc.sel_res_num][i]);
...
void SidechainMove() {
    ...
    do {
        mc.sel_res_num = (int)(threefryrand() * nresidues);
    } while (native_residue[mc.sel_res_num].nrotamers <= 1 ||
             native_residue[mc.sel_res_num].amino_num == 14);

    MakeSidechainMove();
    sidemovedone = 1;
    UpdateLattice(rotate_sidechain_natoms[mc.sel_res_num][0],
                  rotate_sidechain_atom[mc.sel_res_num][0]);
    NewDeltaContacts(rotate_sidechain_natoms[mc.sel_res_num][0],
                     rotate_sidechain_atom[mc.sel_res_num][0],
                     &sidechain_not_rotated[mc.sel_res_num][0]);
