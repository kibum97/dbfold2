            ...
            if (data[i][j].check_hbond)
                if (data[i][j].hbond != NO_HBOND) {
                    if (abs(native[i].res_num - native[j].res_num) > 4) {
                        if (d_memory < HB_INNER * HB_INNER) {
                            e += beta_favor *
                                 seq_hb[helix_sheet][GetAminoNumber(native[i].res)]
                                       [GetAminoNumber(native[j].res)] *
                                 hbond_E[data[i][j].hbond];
                        } else
                        ...
                        if (d_memory < HB_INNER * HB_INNER)
                            e += seq_hb[helix_sheet][GetAminoNumber(native[i].res)]
                                       [GetAminoNumber(native[j].res)] *
                                 hbond_E[data[i][j].hbond];
                        else
                            e += HB_PENALTY *
...
float FoldHydrogenBonds() {
    ...
    for (res_n = 1; res_n < nresidues - 1; res_n++) {
        if (native_residue[res_n].res == "LNK") {
            continue;
        }
        i               = native_residue[res_n].N;
        temp_atom       = &native[i];
...
void InitializeHydrogenBonding() {
    ...
    if ((hbonds = (struct hydrogen_bond **)calloc(natoms, sizeof(struct hydrogen_bond *))) ==
        NULL) {
                ...
                for (j = 0; j < nresidues; j++) {
                    /* *******
                             ...
                             if (strcmp(res, "XXX") && strcmp(res2, "XXX")) {
                               if(!strcmp(native_residue[i].res, res) && !strcmp(native[j].res,
                       res2)) match = 1;
                             }
                    ...
                    for (jj5 = 0; jj5 < LTOR; jj5++)
                        for (jj6 = 0; jj6 < LANG; jj6++)
                            for (jj7 = 0; jj7 < LANG; jj7++)
                                hbond_E[jj1 * i6 + jj2 * i5 + jj3 * i4 + jj4 * i3 + jj5 * i2 +
                                        jj6 * i1 + jj7] = 0;
...
long int CheckHBond(int A, int B) {
    ...
    if (((cur_hbond->donor)->res_num == 0) || ((cur_hbond->acceptor)->res_num == 0))
        return NO_HBOND;
    if (((cur_hbond->donor)->res_num == nresidues - 1) ||
        ((cur_hbond->acceptor)->res_num == nresidues - 1))
        return NO_HBOND;
    ...
    if (res_dif > 4) {
        ...
        if ((secstr[(cur_hbond->donor)->res_num] == 'H') ||
            (secstr[(cur_hbond->acceptor)->res_num] == 'H'))
            return NO_HBOND;
    }
    ...
    // h-bond
    {
        if ((secstr[(cur_hbond->donor)->res_num] == 'E') ||
            (secstr[(cur_hbond->acceptor)->res_num] == 'E')) {
        ...
        if ((secstr[(cur_hbond->donor)->res_num] == 'L') ||
            (secstr[(cur_hbond->acceptor)->res_num] == 'L')) {
            if ((ang_Dphi < 180.) && (ang_Dpsi < 180.))
                return NO_HBOND;
            if ((ang_Aphi < 180.) && (ang_Apsi < 180.))
                return NO_HBOND;
