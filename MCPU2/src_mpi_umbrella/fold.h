...
void Fold(void)
{
    ...
    for (mcstep = 0; mcstep < MC_STEPS; mcstep++)
    {
        ...
        if (mcstep % MC_REPLICA_STEPS == 0)
        {
            ...
            if (myrank == 0)
            { // node 0 will mediate the exchange
                for (irep = 0; irep < MAX_EXCHANGE; irep++)
                {
                    ...
                    if (sel_num % NODES_PER_TEMP == NODES_PER_TEMP - 1)
                    { // the last setpoint for a given temperature
                    ...
                    { // we either exchnage with the node that is one temperature above or one contact setpoint below, with 50% probability each
                        if (threefryrand() < 0.5)
                        {
                            partner = sel_num + 1;
                        }
            ...
            for (i = 0; i < natoms; i++)
            { // Temporary arrays to store all atom coordinates as they stood before any exchanges happened...these will be transferred later
            ...
            for (i = 0; i < nprocs; i++)
            { // normally, replica_index[i] should equal i, unless an exchange occurred
                if (replica_index[i] != i)
                { // there is a discrepancy, indicating an exchange occurred...for instance, replica_index[5]=6 and replica_index[6]=5
                    if (myrank == i)
                    {                                                                                                           // For instance, if my rank is 6 and replica_index[6]=5, then I received an exchange from node 5
                    ...
                    {                                                                                // For instance, if my rank is 5 and replica_index[6]=5, then this means I gave an exchange to node 6
            ...
            { // if exchange occurred, transfer temporary data to real-deal atomic data structure, and recompute energies with the updated atomic info
                for (i = 0; i < natoms; i++)
                {
