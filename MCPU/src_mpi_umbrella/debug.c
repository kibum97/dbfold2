    ...
    for (i = 0; i < natoms; i++)
        for (j = i + 1; j < natoms; j++)
            if (data[i][j].clashes != debug_clashes[i][j])
                fprintf(STATUS, "CLASH_MISMATCH\t(%d, %d)\t%d\t%d\n", i, j, data[i][j].clashes,
                        debug_clashes[i][j]);
    for (i = 0; i < natoms; i++)
        for (j = i + 1; j < natoms; j++)
            if (data[i][j].contacts != debug_contacts[i][j]) {
                fprintf(STATUS, "CONTACT_MISMATCH\t(%d, %d)\t%d\t%d", i, j, data[i][j].contacts,
                        debug_contacts[i][j]);
...
void CheckForDebugContacts(int a, int b) {
    ...
    if (data[a][b].check_contacts &&
        (distance <= contact_distance[native[a].smogtype][native[b].smogtype].b) &&
    ...
    if (debug_contacts[a][b] != data[a][b].contacts)
        fprintf(STATUS, "CONTACTS %d, %d --> %f\t%f\t%f\n", a, b, sqrt(distance) / 100,
                sqrt(hard_core[native[a].smogtype][native[b].smogtype]) / 100,
                sqrt(contact_distance[native[a].smogtype][native[b].smogtype].b) / 100);
    if (debug_clashes[a][b] != data[a][b].clashes)
        fprintf(STATUS, "CLASHES %d, %d --> %ld\t%ld, %ld\n", a, b, distance,
                hard_core[native[a].smogtype][native[b].smogtype],
                contact_distance[native[a].smogtype][native[b].smogtype].b);
