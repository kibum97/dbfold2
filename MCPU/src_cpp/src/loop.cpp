    ...
    float step_size, int *n_soln)  // Does a local move...based on the Dill paper
{
    ...
    *n_soln = 0;  // Number of solutions to 16th degree polynomial
    ...
    sel_res =
        (int)(threefryrand() * top->n_movable_residues);  // Select a residue to initialize local move
    sel_res = top->movable_residue_map[sel_res];          // KP editted for movable region constraint
                                                     //  sel_res = 20;
    ctx->mc.is_phi = (int)(threefryrand() *
                      2);  // With 50% probability, rotate about phi angle, otherwise psi angle
    step_size =
        integrator->YANG_SCALE * 2. * deg2rad * GaussianNum();  // Magnitude  of driver rotation, in radians
...
void get_orgco(
    ...
    for (cur_res = 0; cur_res < 5; cur_res++)
        for (cur_atom = top->res_atomno[n0 - 1]; cur_atom < top->res_atomno[n0 + 4]; cur_atom++)
...
void get_orgat(
    ...
    if (strcmp(ctx->native[a].atomname, "N") == 0) {
        rorg_n[r][0] = ctx->orig_native[a].xyz.x;
        rorg_n[r][1] = ctx->orig_native[a].xyz.y;
        rorg_n[r][2] = ctx->orig_native[a].xyz.z;
    } else if (strcmp(ctx->native[a].atomname, "CA") == 0) {
        rorg_a[r][0] = ctx->orig_native[a].xyz.x;
        rorg_a[r][1] = ctx->orig_native[a].xyz.y;
        rorg_a[r][2] = ctx->orig_native[a].xyz.z;
    } else if (strcmp(ctx->native[a].atomname, "C") == 0) {
        rorg_c[r][0] = ctx->orig_native[a].xyz.x;
        rorg_c[r][1] = ctx->orig_native[a].xyz.y;
        rorg_c[r][2] = ctx->orig_native[a].xyz.z;
    } else if (strcmp(ctx->native[a].atomname, "O") == 0) {
        rorg_o[r][0] = ctx->orig_native[a].xyz.x;
        rorg_o[r][1] = ctx->orig_native[a].xyz.y;
        rorg_o[r][2] = ctx->orig_native[a].xyz.z;
    } else if (strcmp(ctx->native[a].atomname, "O") == 0) {
        rorg_o[r][0] = ctx->orig_native[a].xyz.x;
        rorg_o[r][1] = ctx->orig_native[a].xyz.y;
        rorg_o[r][2] = ctx->orig_native[a].xyz.z;
    } else {
        rorg_s[r][ns[r]][0] = ctx->orig_native[a].xyz.x;
        rorg_s[r][ns[r]][1] = ctx->orig_native[a].xyz.y;
        rorg_s[r][ns[r]][2] = ctx->orig_native[a].xyz.z;
        ns[r]++;
    }
    return;
}
...
void get_atom(
    ...
    if (strcmp(ctx->native[a].atomname, "N") == 0) {
        r_n[r][0] = ctx->native[a].xyz.x;
        r_n[r][1] = ctx->native[a].xyz.y;
        r_n[r][2] = ctx->native[a].xyz.z;
    } else if (strcmp(ctx->native[a].atomname, "CA") == 0) {
        r_a[r][0] = ctx->native[a].xyz.x;
        r_a[r][1] = ctx->native[a].xyz.y;
        r_a[r][2] = ctx->native[a].xyz.z;
    } else if (strcmp(ctx->native[a].atomname, "C") == 0) {
        r_c[r][0] = ctx->native[a].xyz.x;
        r_c[r][1] = ctx->native[a].xyz.y;
        r_c[r][2] = ctx->native[a].xyz.z;
    } else if (strcmp(ctx->native[a].atomname, "O") == 0) {
        r_o[r][0] = ctx->native[a].xyz.x;
        r_o[r][1] = ctx->native[a].xyz.y;
        r_o[r][2] = ctx->native[a].xyz.z;
    } else if (strcmp(ctx->native[a].atomname, "O") == 0) {
        r_o[r][0] = ctx->native[a].xyz.x;
        r_o[r][1] = ctx->native[a].xyz.y;
        r_o[r][2] = ctx->native[a].xyz.z;
    } else {
        r_s[r][ns[r]][0] = ctx->native[a].xyz.x;
        r_s[r][ns[r]][1] = ctx->native[a].xyz.y;
        r_s[r][ns[r]][2] = ctx->native[a].xyz.z;
        ns[r]++;
    }
    return;
}
...
void put_atom(struct Context *ctx,
              ...
              double r_s[5][MSAR][3], int ns[5], int r, int a) {
    if (strcmp(ctx->native[a].atomname, "N") == 0) {
        ctx->native[a].xyz.x = r_n[r][0];
        ctx->native[a].xyz.y = r_n[r][1];
        ctx->native[a].xyz.z = r_n[r][2];
    } else if (strcmp(ctx->native[a].atomname, "CA") == 0) {
        ctx->native[a].xyz.x = r_a[r][0];
        ctx->native[a].xyz.y = r_a[r][1];
        ctx->native[a].xyz.z = r_a[r][2];
    } else if (strcmp(ctx->native[a].atomname, "C") == 0) {
        ctx->native[a].xyz.x = r_c[r][0];
        ctx->native[a].xyz.y = r_c[r][1];
        ctx->native[a].xyz.z = r_c[r][2];
    } else if (strcmp(ctx->native[a].atomname, "O") == 0) {
        ctx->native[a].xyz.x = r_o[r][0];
        ctx->native[a].xyz.y = r_o[r][1];
        ctx->native[a].xyz.z = r_o[r][2];
    } else if (strcmp(ctx->native[a].atomname, "O") == 0) {
        ctx->native[a].xyz.x = r_o[r][0];
        ctx->native[a].xyz.y = r_o[r][1];
        ctx->native[a].xyz.z = r_o[r][2];
    } else {
        ctx->native[a].xyz.x = r_s[r][ns[r]][0];
        ctx->native[a].xyz.y = r_s[r][ns[r]][1];
        ctx->native[a].xyz.z = r_s[r][ns[r]][2];
        ns[r]++;
    }
    return;
}
...
void yangloop(
    ...
    double r_soln_n[max_soln][3][3], r_soln_a[max_soln][3][3], r_soln_c[max_soln][3][3],
        r_soln_o[max_soln][3][3], r_soln_s[3][MSAR][3];
    ...
    */
    ...
    initialize_loop_closure(
        b_len, b_ang,
        t_ang);  // Feed this function values that are to be fixed (by basic chemistry), namely bond
                 // angles, bond lengths, and the omega dihedral angle, always 180 degrees
    solve_3pep_poly(r_n[1], r_a[1], r_a[3], r_c[3], r_soln_n, r_soln_a, r_soln_c,
                    &n_soln_before);  // This is the function that solves the loop closture
                                      // problem..Need to feed it coordinates for nitrogen/C_alpha
                                      // atoms in first and third residue...see tripep_closure.h
    ctx->soln_no_before = n_soln_before;   // Although n_soln_before is initialized to zero, its value
                                      // gets updated in the above solve_3p3_poly
    if (ctx->soln_no_before == 0)
        ctx->soln_no_before = 1;
    for (i = 0; i < 3; i++)
        for (k = 0; k < 3;
             k++)  // Keep track of the atom positions that we just computed, prior to rotation
        {
            r_n_before[i][k]  = r_n[i + 1][k];
            r_ca_before[i][k] = r_a[i + 1][k];
            r_c_before[i][k]  = r_c[i + 1][k];
        }
    loop_Jacobian(r_n_before, r_ca_before, r_c_before,
                  &before_jacobi);  // Compute Jacobi pre-rotation
    ...
    If the later, we change position of first N and CA (although I had somehow thoguht these were
    fixed..)
    */
    if (ctx->mc.is_phi)  // We rotate the phi dihedral, which goes from N to CA
    {
        driver_rot(r_c[3], r1_c[3], r_a[4], r_n[4],
                   dih_ch);  // We plug in the current value of the carbon, and get the updated
                             // value (second argument) as a result of a dihedral rotation of value
                             // dih_ch...honeslty not totally clear how this works from Dill paper..
    ...
    solve_3pep_poly(r_n[1], r_a[1], r_a[3], r_c[3], r_soln_n, r_soln_a, r_soln_c,
                    n_soln);  // Find new loop closure solution post rotation

    if (calc_rmsd)  // This clause is ridiculous since calc_rmsd is initialized to 1 before so will
                    // always be true...but it's an important clause since in it, you choose the
                    // solution to accept etc...this can be cleaned up but I didn't want to mess
                    // with code too much...
    {
        for (k = 0; k < *n_soln; k++) {
            for (i = 0; i < 3; i++)
        ...
        // AB decided to move the Jacobi calculation to later, at which point there is a loop that
        // naturlaly allows r_n_after etc to be defined

        //       if(*n_soln>0)
        if ((*n_soln > 0) && (*n_soln <= deg_pol))  // This is a huge clause, and I did not look at
                                                    // what it does, althoguh I should
        {
            if (ctx->mc.is_phi)
            // This is the case where driver angle is res. 4. (0~4)
            {
                // rotation with n0-a0
                for (z = 0; z < ns[1]; z++)
                    get_rot(r0_s[0][z], r_soln_s[0][z], r_c[0], r_soln_n[index][0],
                            r_soln_a[index][0], r0_c[0], r_soln_c[index][0]);
                get_rot(r0_o[0], rot1_o[0], r_c[0], r_soln_n[index][0], r_soln_a[index][0], r0_c[0],
                        r_soln_c[index][0]);
                get_rot(r0_n[1], rot1_n[1], r_c[0], r_soln_n[index][0], r_soln_a[index][0], r0_c[0],
                        r_soln_c[index][0]);
                get_rot(r0_a[1], rot1_a[1], r_c[0], r_soln_n[index][0], r_soln_a[index][0], r0_c[0],
                        r_soln_c[index][0]);
                for (z = 0; z < ns[2]; z++)
                    get_rot(r0_s[1][z], rot1_s[1][z], r_c[0], r_soln_n[index][0],
                            r_soln_a[index][0], r0_c[0], r_soln_c[index][0]);
                get_rot(r0_c[1], rot1_c[1], r_c[0], r_soln_n[index][0], r_soln_a[index][0], r0_c[0],
                        r_soln_c[index][0]);

                // rotation with a0-c0
                get_rot(rot1_o[0], r_soln_o[index][0], r_soln_n[index][0], r_soln_a[index][0],
                        r_soln_c[index][0], rot1_n[1], r_soln_n[index][1]);
                get_rot(rot1_a[1], rot2_a[1], r_soln_n[index][0], r_soln_a[index][0],
                        r_soln_c[index][0], rot1_n[1], r_soln_n[index][1]);
                for (z = 0; z < ns[2]; z++)
                    get_rot(rot1_s[1][z], rot2_s[1][z], r_soln_n[index][0], r_soln_a[index][0],
                            r_soln_c[index][0], rot1_n[1], r_soln_n[index][1]);
                get_rot(rot1_c[1], rot2_c[1], r_soln_n[index][0], r_soln_a[index][0],
                        r_soln_c[index][0], rot1_n[1], r_soln_n[index][1]);

                // rotation with c0-n1
                for (z = 0; z < ns[2]; z++)
                    get_rot(rot2_s[1][z], rot3_s[1][z], r_soln_a[index][0], r_soln_c[index][0],
                            r_soln_n[index][1], rot2_a[1], r_soln_a[index][1]);
                get_rot(rot2_c[1], rot3_c[1], r_soln_a[index][0], r_soln_c[index][0],
                        r_soln_n[index][1], rot2_a[1], r_soln_a[index][1]);

                // rotation with n1-a1
                for (z = 0; z < ns[2]; z++)
                    get_rot(rot3_s[1][z], r_soln_s[1][z], r_soln_c[index][0], r_soln_n[index][1],
                            r_soln_a[index][1], rot3_c[1], r_soln_c[index][1]);
                ...
                // rotation with c2-a2
                for (z = 0; z < ns[3]; z++)
                    get_rot(rot1_s[2][z], r_soln_s[2][z], r_n[4], r_soln_c[index][2],
                            r_soln_a[index][2], rot1_n[2], r_soln_n[index][2]);
                get_rot(rot1_c[1], rot2_c[1], r_n[4], r_soln_c[index][2], r_soln_a[index][2],
                        rot1_n[2], r_soln_n[index][2]);
                get_rot(rot1_o[1], rot2_o[1], r_n[4], r_soln_c[index][2], r_soln_a[index][2],
                        rot1_n[2], r_soln_n[index][2]);
                get_rot(rot1_a[1], rot2_a[1], r_n[4], r_soln_c[index][2], r_soln_a[index][2],
                        rot1_n[2], r_soln_n[index][2]);

                // rotation with a2-n2
                get_rot(rot2_o[1], rot3_o[1], r_soln_c[index][2], r_soln_a[index][2],
                        r_soln_n[index][2], rot2_c[1], r_soln_c[index][1]);
                get_rot(rot2_a[1], rot3_a[1], r_soln_c[index][2], r_soln_a[index][2],
                        r_soln_n[index][2], rot2_c[1], r_soln_c[index][1]);

                // rotation with n2-c1
                get_rot(rot3_o[1], r_soln_o[index][1], r_soln_a[index][2], r_soln_n[index][2],
                        r_soln_c[index][1], rot3_a[1], r_soln_a[index][1]);
            } else
            // This is the case where driver angle is res. 0. (0~4)
            {
                ...
                // rotation with n0-a0
                for (z = 0; z < ns[1]; z++)
                    get_rot(rot1_s[0][z], r_soln_s[0][z], r_c[0], r_soln_n[index][0],
                            r_soln_a[index][0], rot1_c[0], r_soln_c[index][0]);
                get_rot(rot1_o[0], rot2_o[0], r_c[0], r_soln_n[index][0], r_soln_a[index][0],
                        rot1_c[0], r_soln_c[index][0]);
                get_rot(rot1_n[1], rot2_n[1], r_c[0], r_soln_n[index][0], r_soln_a[index][0],
                        rot1_c[0], r_soln_c[index][0]);
                get_rot(rot1_a[1], rot2_a[1], r_c[0], r_soln_n[index][0], r_soln_a[index][0],
                        rot1_c[0], r_soln_c[index][0]);
                for (z = 0; z < ns[2]; z++)
                    get_rot(rot1_s[1][z], rot2_s[1][z], r_c[0], r_soln_n[index][0],
                            r_soln_a[index][0], rot1_c[0], r_soln_c[index][0]);
                get_rot(rot1_c[1], rot2_c[1], r_c[0], r_soln_n[index][0], r_soln_a[index][0],
                        rot1_c[0], r_soln_c[index][0]);

                // rotation with a0-c0
                get_rot(rot2_o[0], r_soln_o[index][0], r_soln_n[index][0], r_soln_a[index][0],
                        r_soln_c[index][0], rot2_n[1], r_soln_n[index][1]);
                get_rot(rot2_a[1], rot3_a[1], r_soln_n[index][0], r_soln_a[index][0],
                        r_soln_c[index][0], rot2_n[1], r_soln_n[index][1]);
                for (z = 0; z < ns[2]; z++)
                    get_rot(rot2_s[1][z], rot3_s[1][z], r_soln_n[index][0], r_soln_a[index][0],
                            r_soln_c[index][0], rot2_n[1], r_soln_n[index][1]);
                get_rot(rot2_c[1], rot3_c[1], r_soln_n[index][0], r_soln_a[index][0],
                        r_soln_c[index][0], rot2_n[1], r_soln_n[index][1]);

                // rotation with c0-n1
                for (z = 0; z < ns[2]; z++)
                    get_rot(rot3_s[1][z], rot4_s[1][z], r_soln_a[index][0], r_soln_c[index][0],
                            r_soln_n[index][1], rot3_a[1], r_soln_a[index][1]);
                get_rot(rot3_c[1], rot4_c[1], r_soln_a[index][0], r_soln_c[index][0],
                        r_soln_n[index][1], rot3_a[1], r_soln_a[index][1]);

                // rotation with n1-a1
                for (z = 0; z < ns[2]; z++)
                    get_rot(rot4_s[1][z], r_soln_s[1][z], r_soln_c[index][0], r_soln_n[index][1],
                            r_soln_a[index][1], rot4_c[1], r_soln_c[index][1]);
                ...
                // rotation with c2-a2
                for (z = 0; z < ns[3]; z++)
                    get_rot(r0_s[2][z], r_soln_s[2][z], r_n[4], r_soln_c[index][2],
                            r_soln_a[index][2], r0_n[2], r_soln_n[index][2]);
                get_rot(r0_c[1], rot1_c[1], r_n[4], r_soln_c[index][2], r_soln_a[index][2], r0_n[2],
                        r_soln_n[index][2]);
                get_rot(r0_o[1], rot1_o[1], r_n[4], r_soln_c[index][2], r_soln_a[index][2], r0_n[2],
                        r_soln_n[index][2]);
                get_rot(r0_a[1], rot1_a[1], r_n[4], r_soln_c[index][2], r_soln_a[index][2], r0_n[2],
                        r_soln_n[index][2]);

                // rotation with a2-n2
                get_rot(rot1_o[1], rot2_o[1], r_soln_c[index][2], r_soln_a[index][2],
                        r_soln_n[index][2], rot1_c[1], r_soln_c[index][1]);
                get_rot(rot1_a[1], rot2_a[1], r_soln_c[index][2], r_soln_a[index][2],
                        r_soln_n[index][2], rot1_c[1], r_soln_c[index][1]);

                // rotation with n2-c1
                get_rot(rot2_o[1], r_soln_o[index][1], r_soln_a[index][2], r_soln_n[index][2],
                        r_soln_c[index][1], rot2_a[1], r_soln_a[index][1]);
            }
...
void check_phipsi(
    ...
    for (i = 0; i < top->nresidues; i++) {
        res_N[i][0]  = ctx->native[ctx->native_residue[i].N].xyz.x;
        res_N[i][1]  = ctx->native[ctx->native_residue[i].N].xyz.y;
        res_N[i][2]  = ctx->native[ctx->native_residue[i].N].xyz.z;
        res_CA[i][0] = ctx->native[ctx->native_residue[i].CA].xyz.x;
        res_CA[i][1] = ctx->native[ctx->native_residue[i].CA].xyz.y;
        res_CA[i][2] = ctx->native[ctx->native_residue[i].CA].xyz.z;
        res_C[i][0]  = ctx->native[ctx->native_residue[i].C].xyz.x;
        res_C[i][1]  = ctx->native[ctx->native_residue[i].C].xyz.y;
        res_C[i][2]  = ctx->native[ctx->native_residue[i].C].xyz.z;
    }

    for (i = 0; i < top->nresidues - 2; i++) {
        c_dih_ang(res_C[i], res_N[i + 1], res_CA[i + 1], res_C[i + 1], &ctx->cur_phi[i]);
        c_dih_ang(res_N[i + 1], res_CA[i + 1], res_C[i + 1], res_N[i + 2], &ctx->cur_psi[i]);
