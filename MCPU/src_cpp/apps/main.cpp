...
int main(int argc, char *argv[]) {
        ...
        /* Get the communicator sizes and ranks */
        int nprocs, myrank;
        if (MPI_Comm_size(MPI_COMM_WORLD, &nprocs) != MPI_SUCCESS) {
            fprintf(stderr, "Error getting MPI communicator size.\n");
            MPI_Finalize();
            return 1;
        } else if (MPI_Comm_rank(MPI_COMM_WORLD, &myrank) != MPI_SUCCESS) {
    ...
    if (argc != 2) {
        ...
        std::runtime_error("Incorrect number of command-line arguments.");
    }
    /* Initialize simulation defaults */
    //BootSimulationDefaults();
    ConfigLoader config_loader;
    config_loader.read(argv[1]);
    printf("Simulation defaults initialized.\n");
    
    sim.STATUS = stdout;
    // sim.mpi_world_comm = MPI_COMM_WORLD;

    /* current_replica tracks a set of coordinates across replica space */
    sim.current_replica = sim.myrank;
    printf("Current replica set to %d.\n", sim.current_replica);
    /* initialize data */
    SetProgramOptions(&sim, &sys, &integrator, &ctx, &top, argc, argv);
    std::cout << "Debug: Program options set." << std::endl;
    printf("Program options set.\n");
    sim.seed = time(NULL);
    ...
    std::cout << "DEBUG: movable residue file is " << sim.movable_region_file << std::endl;
    std::cout << "DEBUG: rmsd constraint file is " << sim.rmsd_constraint_file << std::endl;
    if (!sim.rmsd_constraint_file.empty() && sim.rmsd_constraint_file != "None") {
        std::cout << "DEBUG: Initializing RMSD constraints." << std::endl;
        Init_frag_rmsd_constraints(&sim);
    }
    if (!sim.movable_region_file.empty() && sim.movable_region_file != "None") {  // KP added for movable region constraint
        std::cout << "DEBUG: Initializing movable region." << std::endl;
